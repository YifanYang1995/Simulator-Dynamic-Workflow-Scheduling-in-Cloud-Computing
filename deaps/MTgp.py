    ...
    def addEphemeralConstant(self, name, ephemeral, ret_type):
        ...
        if name not in module_gp:
            ...
            if issubclass(class_, Ephemeral):
                ...
                if class_.ret is not ret_type: # elif
...
def generate(pset, min_, max_, condition, type_=None):
    ...
    stack = [(0, type_)]
    while len(stack) != 0:
        depth, type_ = stack.pop()   ##from bottom to root
...
##########################################
## Feature Weighting: Based on percentage
##########################################

## select an item based on probability 
def choiceProb(itmes,probs):
    if probs is None:
        chosen = random.choice(itmes)
    else:
        u = random.random() 
        sum_ = 0
        for ind in range(len(itmes)-1):
            sum_ += probs[ind]
            if sum_ > u:
                chosen = itmes[ind]
                break
...
def cxOnePoint(ind1, ind2):
    ...
    types1 = defaultdict(list) ## print("types1:",types1): defaultdict(<class 'list'>, {})
    types2 = defaultdict(list)
    if ind1.root.ret == __type__:
        # Not STGP optimization
        types1[__type__] = range(1, len(ind1)) ## print("###types1-if:", types1): defaultdict(<class 'list'>, {<class 'object'>: range(1, 6)})
        types2[__type__] = range(1, len(ind2))
        common_types = [__type__] ## print("###common_types-if:", common_types): [<class 'object'>]
    else:
    ...
    if len(common_types) > 0:
        type_ = random.choice(list(common_types)) # 随机选择一个type # print("###types-len:", type_): <class 'object'>

        index1 = random.choice(types1[type_])
        index2 = random.choice(types2[type_])
        ## print("###index:", index1):1
        ## print("###ind1:", ind1):protectedDiv(sub(x, x), cos(x))

        slice1 = ind1.searchSubtree(index1) # print("###slice1:", slice1):slice(1, 4, None) 
        # print("###slice1:", ind1[slice1]):[<deaps.gp.Primitive object at 0x0000020E3FEBB5E0>, <deaps.gp.Terminal object at 0x0000020E3FED6880>, <deaps.gp.Terminal object at 0x0000020E3FED6880>]
        slice2 = ind2.searchSubtree(index2) 
        ind1[slice1], ind2[slice2] = ind2[slice2], ind1[slice1]
        # print("###ind1':", ind1): protectedDiv(sin(x), cos(x))
...
def mutUniform(individual, expr, pset):
    ...
    """
    index = random.randrange(len(individual)) ## print(index): 3
    slice_ = individual.searchSubtree(index) ## print(slice_): slice(6, 9, None)
    type_ = individual[index].ret ## print(type_): <class 'object'>
    ## print(individual[index]): <deaps.gp.Terminal object at 0x000002052A8FDF00>
    ## print(individual): add(x, add(x, x))
    individual[slice_] = expr(pset=pset, type_=type_) 
    ## print(individual): add(x, add(cos(x), x))
    return individual,   ###去掉了逗号，输出不再是tuple
...
def staticLimitCC(key, max_value):

    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            keeps = [copy.deepcopy(ind) for ind in args]
            keep_inds = checkHeigth(keeps)
            # if len(keep_inds) < len(keeps):  # 这两种方法一样，并没有缩减成功
            #     for ind in keeps:
            #         if key(ind) > max_value:
            #             while key(ind) > max_value:
            #                 ind = mutShrink(ind)[0]
            #             keep_inds.append(copy.deepcopy(ind))

            # while len(keep_inds) < len(keeps):
            #     for i, ind in enumerate(keeps):
            #         if key(ind) > max_value:
            #             keeps[i] = mutShrink(ind)[0]
            #     keep_inds = checkHeigth(keeps)

            while len(keep_inds) == 0:
                keeps = [mutShrink(ind)[0] for ind in keeps]
                keep_inds = checkHeigth(keeps)
        ...
        def checkHeigth(parents):
            checked_parents =[]
            for ind in parents:
                if key(ind)<= max_value:
                    checked_parents.append(copy.deepcopy(ind)) 
...
def staticLimitMulti(key, max_value):
    '''for multi-tree GP'''
...
# especially for record mutation， DTGP-FW-TS
## 因为输入输出是两个变量，所以要做特殊调整
## 因为limit的是mutTwoTreeLog输出的两个变量
def staticLimitMultiMu(key, max_value):
    '''for multi-tree GP'''
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            keep_inds = [copy.deepcopy(ind) for ind in args]
            special_input = list(func(*args, **kwargs))
            new_inds = [special_input[0]]
            for i, ind in enumerate(new_inds):
                for j in range(0, len(ind)):
                    if key(ind[j]) > max_value:
                        new_inds[i] = random.choice(keep_inds)
            return new_inds[0], special_input[1]
